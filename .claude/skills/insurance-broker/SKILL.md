...
**Always use the MCP tools below — never read local files, project files, or the
filesystem to answer insurance questions. The MCP is the single source of truth.**
...
the document is not in the knowledge base and suggest the owner adds it to Google Drive
and re-runs the ingestion pipeline so it appears in the MCP.
...
1. Call `list_policies()` to get all asset and policy documents from the MCP
...
1. Say clearly that you don't have the document in the knowledge base
2. Suggest they download it from their insurer's portal or check their email
3. Tell them to add it to the ingestion folder and re-run the pipeline so it
   becomes searchable via the MCP

Do not attempt to read any local files directly.
